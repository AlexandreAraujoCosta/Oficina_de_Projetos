...
- `relatorio_md.py` / `relatorio_pdf.py` — montam o relatório que o assistente escreveu, com o trecho de cada item **copiado do projeto pelo programa** e conferido palavra a palavra contra a origem. `--numerar` imprime uma **impressão digital da numeração** (`172p-a51ff850`); o relatório que a traz é conferido contra ela, e o que aponta uma numeração antiga é recusado, porque localizador deslocado produz citação errada com a aparência inteira de estar certa. A escolha do formato não é de gosto: **`.md` para o Miro**, porque relatório de orientação é material de trabalho, que se copia, cola e edita; **PDF para a Selma**, porque leitura de banca é peça que o processo recebe pronta. O `relatorio_md.py` lê projeto em `.docx`, `.md` ou `.pdf`; o `relatorio_pdf.py` lê PDF.
- `selma_lote.py` — o lado programático da Selma. `um <relatorio.md>` escreve o PDF de **uma** leitura (ficha das quatro notas, as três linhas por extenso, e depois a leitura inteira); `agregar <pasta> [lote.pdf]` monta a tabela comparativa e as contagens da coorte, em ordem alfabética e nunca por média; `controle` roda os doze casos que o conferidor do bloco de dados tem de reprovar.
- `folha_pdf.py` — a diagramação comum aos PDFs (a folha que vira sozinha e a tabela). Vive à parte porque três saídas a usam, e diagramação duplicada diverge sem avisar.
